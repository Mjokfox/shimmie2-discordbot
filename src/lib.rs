...
pub fn create_post(conn: &mut SqliteConnection, post_id: &i32, message_id: &i64) -> Post {
...
pub fn delete_post(conn: &mut SqliteConnection, p_id: &i32) {
    ...
    diesel::delete(posts.filter(post_id.eq(p_id))).execute(conn).expect("Error deleting post");
}
...
pub fn create_comment(conn: &mut SqliteConnection, comment_id: &i32, post_id: &i32, message_id: &i64) -> Comment {
...
pub fn delete_comment(conn: &mut SqliteConnection, c_id: &i32) {
    ...
    diesel::delete(comments.filter(comment_id.eq(c_id))).execute(conn).expect("Error deleting comment");
}
...
pub fn delete_comments_with_post_id(conn: &mut SqliteConnection, p_id: &i32) {
    ...
    diesel::delete(comments.filter(post_id.eq(p_id))).execute(conn).expect("Error deleting comments");
}